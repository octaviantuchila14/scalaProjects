...
class BinaryTreeSuite(_system: ActorSystem) extends TestKit(_system) with FunSuiteLike with Matchers with BeforeAndAfterAll with ImplicitSender
{
  ...
  def verifyWithGC(probe: TestProbe, ops: Seq[Operation], expected: Seq[OperationReply]): Unit = {
  ...
  test("instruction with GC example") {
    val requester = TestProbe()
    val requesterRef = requester.ref
    val ops = List(
      Insert(requesterRef, id=100, 1),
  ...
  test("more instruction with GC example") {
    val requester = TestProbe()
    val requesterRef = requester.ref
    val ops = List(
      ...
      Insert(requesterRef, id=40, 4),

      Insert(requesterRef, id=50, 5),
      Insert(requesterRef, id=60, 6),

      Remove(requesterRef, id=70, 1),
      Remove(requesterRef, id=80, 2),

      Remove(requesterRef, id=90, 3),
      Remove(requesterRef, id=100, 4),

      Contains(requesterRef, id=110, 1),
      Contains(requesterRef, id=120, 2),
      Contains(requesterRef, id=130, 3),
      Contains(requesterRef, id=140, 4),
      Contains(requesterRef, id=150, 5),
      Contains(requesterRef, id=160, 6)
    )
    ...
    val expectedReplies = List(
      ...
      OperationFinished(id=70),
      OperationFinished(id=80),
      OperationFinished(id=90),
      OperationFinished(id=100),
      ContainsResult(id=110, false),
      ContainsResult(id=120, false),
      ContainsResult(id=130, false),
      ContainsResult(id=140, false),
      ContainsResult(id=150, true),
      ContainsResult(id=160, true)
    )
