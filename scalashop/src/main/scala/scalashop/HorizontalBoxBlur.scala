...
object HorizontalBoxBlur {
  ...
  def blur(src: Img, dst: Img, from: Int, end: Int, radius: Int): Unit = {
    val computations = (from until end).map(y => {
      task{
        (0 until src.height).foreach(x => {
//          println("updating for: " + x + " " + y)
  ...
  def parBlur(src: Img, dst: Img, numTasks: Int, radius: Int): Unit = {
    val strips: List[(Int, Int)] = (0 until numTasks).map(i => (i * (src.height/numTasks), Math.min((i + 1)*(src.height/numTasks), src.height))).toList
