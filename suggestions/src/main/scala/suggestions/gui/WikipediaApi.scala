...
//     */
//    def sanitized: Observable[String] = obs.map(event => event.replaceAll("\\s", "_"))
//
//  }
//
...
//     */
//    def recovered: Observable[Try[T]] = obs.map(_ match {
...
//     */
//    def timedOut(totalSec: Long): Observable[T] = obs.takeUntil(Observable.timer(Duration.fromNanos(totalSec * 1000)))
//
//
