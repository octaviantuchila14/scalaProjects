...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val noEmpt: GenMap[Point, GenSeq[Point]] = points.par.groupBy(point => findClosest(point, means))
    means.foldLeft(noEmpt) { (groups, mean) => {
      if(!groups.contains(mean)) groups + (mean -> GenSeq())
      else groups
    }}
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.par.zip(newMeans).forall{case(o, n) => (Math.pow(o.x - n.x, 2) + Math.pow(o.y - n.y, 2) + Math.pow(o.z - n.z, 2) < eta)}
  }
